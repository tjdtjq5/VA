...
namespace ImaginationOverflow.UniversalDeepLinking.Editor
{
    public class MacOsPosBuild : IPosBuilder
    {
        ...
        private void ExtractFrameworkAndAddItToXcode(string pathToBuiltProject)
        {
            ...
            var pathToLib = Path.Combine(Application.dataPath, EditorHelpers.PluginPath + "/libs/Standalone/Mac/UniversalDeepLink.framework.zip");
        ...
        private void AddScriptsToIncludeFramework(string pathToBuiltProject)
        {
            ...
            File.Copy(Path.Combine(Application.dataPath, EditorHelpers.PluginPath + "/libs/Standalone/Mac/UniversalDeepLink.framework.zip"), Path.Combine(dir.FullName, "UniversalDeepLink.framework.zip"), true);
        ...
        private void CopyUniversalDeepLinkFramework(string pathToBuiltProject)
        {
            ...
            var universalDeepLinkFrameworkPath = Path.Combine(Application.dataPath,
                EditorHelpers.PluginPath + "/libs/Standalone/Mac/" + FrameworkName);
        ...
        private static void RunOptool(string optoolPath, string opToolCmd)
        {
            ...
            if (File.Exists(optoolPath) == false)
            {
                Debug.LogError("Could not found optool, depending on your mac version this can be required in order for the plugin to work. " +
                               "You can download it from our GDrive: https://drive.google.com/drive/folders/1nRqRfUfEqTIO2olniRPsUbRY1Nc52QvE");
        ...
        private bool CheckIfIsXcodeBuild(string path)
        {
            ...
            if (files.Length != 1)
            {
                files = files.Where(f => f.EndsWith(".xcodeproj"))
                    .Where(f => !f.StartsWith("GameAssembly"))
                    .ToArray();
            }
