...
namespace Assets.SimpleSignIn.Apple.Scripts
{
    public static class AppleJWT
    {
        public static string CreateClientSecret(AppleAuthSettings settings)
        {
            ...
            var payload = JsonConvert.SerializeObject(new { iss = settings.TeamId, iat = DateTimeOffset.UtcNow.ToUnixTimeSeconds(), exp = DateTimeOffset.UtcNow.ToUnixTimeSeconds() + 15777000, aud = "https://appleid.apple.com", sub = settings.ClientId });
