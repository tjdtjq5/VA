...
namespace OPS.Obfuscator.Editor.Assembly.Mono.Member.Key
{
    ...
    /// </summary>
    public class EventKey : IMemberKey
    {
        // Constructor
        #region Constructor
        ...
        public EventKey(EventReference _EventReference)
        {
            if(_EventReference == null)
            {
                throw new ArgumentNullException("_EventReference");
            }

            try
            {
                EventDefinition var_EventDefinition = _EventReference.Resolve();

                this.Type = new TypeKey(var_EventDefinition.DeclaringType);

                this.FullName = var_EventDefinition.FullName;
                this.Name = _EventReference.Name;
            }
            catch(Exception e)
            {
                throw new Exception("Could not resolve: " + _EventReference.FullName + " in " + _EventReference.DeclaringType.Scope.Name + ". Please add the assemblies directory as dependency. Error: " + e.ToString());
            }
        }

        public EventKey(EventDefinition _EventDefinition)
        {
            if (_EventDefinition == null)
            {
                throw new ArgumentNullException("_EventDefinition");
            }

            this.Type = new TypeKey(_EventDefinition.DeclaringType);

            this.FullName = _EventDefinition.FullName;
            this.Name = _EventDefinition.Name;
        }

        public EventKey(String _Assembly, String _FullName)
        {
            if (_Assembly == null)
            {
        ...
        {
            return this.Equals(_Object as EventKey);
        }

        public bool Equals(EventKey _EventKey)
        {
            return _EventKey != null && this.Assembly.Equals(_EventKey.Assembly) && this.FullName.Equals(_EventKey.FullName);
        }
        ...
        public static bool operator ==(EventKey _A, EventKey _B)
        {
        ...
        public bool Deserialize(string _Value)
        {
            ...
            this.Name = IMemberDefinitionHelper.GetMemberName(EMemberType.Event, var_FullName);
