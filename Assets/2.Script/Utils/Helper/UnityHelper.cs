...
{
    #region Log
    public static void Log_H(string message, params object[] args)
    {
        if (!GameOptionManager.IsRelease)
            Debug.Log(CSharpHelper.Format_H(message, args));
    }
    ...
    #region Transform
    public static void LookAt_H(this Transform tr, Transform target, float addAngle)
    {
        var angle = tr.position.GetAngle(target.position) + addAngle;
        tr.localRotation = Quaternion.Euler(tr.localRotation.x, tr.localRotation.y, angle);
    }
    ...
    public static Vector3 GetAngleMovePos(Vector3 originPos, float angle, float distance)
    {
        Quaternion v3Rotation = Quaternion.Euler(0, 0, angle);
        Vector3 v3Direction = Vector3.up; // forward -> z , up -> y
        Vector3 v3RotatedDirection = v3Rotation * v3Direction * distance;
        v3RotatedDirection = new Vector3(v3RotatedDirection.x, v3RotatedDirection.y, -v3RotatedDirection.z);

        originPos += v3RotatedDirection;
        return originPos;
    }
    #endregion
    ...
    public static SpineAniController Initialize(this SkeletonGraphic animator)
    {
        SpineAniController ac = animator.GetOrAddComponent<SpineAniController>();
        ac.Initialize(animator);
        return ac;
    ...
    public static float Random_H(float minInclusice, float maxExclusice)
    {
        return UnityEngine.Random.Range(minInclusice, maxExclusice);
    }
    #endregion
